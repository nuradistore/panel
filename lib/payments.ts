...
export async function completeAlightMotionPaymentProcessing(transactionId: string, alightMotionDetails: AlightMotionDetails): Promise<boolean> {
  try { const client=await clientPromise; const db=client.db(appConfig.mongodb.dbName); const result=await collection(db).updateOne({transactionId,status:"processing"},{$set:{status:"completed",alightMotionDetails}}); if(result.modifiedCount===1){revalidatePath(`/invoice/${transactionId}`);return true} return false } catch(error){console.error("Error completing AM payment:",error);return false}
}
