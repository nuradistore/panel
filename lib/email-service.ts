...
export async function sendPasswordResetEmail(to: string, token: string) {
  ...
  const mailOptions = {
    from: appConfig.emailSender.from,
    to,
    subject: `Reset Password - ${appConfig.nameHost}`,
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:24px;background:#07101c;color:#e5eefb;border-radius:16px">
        <div style="padding:18px;border-radius:12px;background:linear-gradient(135deg,#06b6d4,#3b82f6,#8b5cf6)">
          <h2 style="margin:0;color:white;text-align:center">Reset Password ${appConfig.nameHost}</h2>
        </div>
        <div style="padding:24px 8px 8px">
          <p style="line-height:1.7">Kami menerima permintaan untuk mengganti password akun ${appConfig.nameHost} Anda.</p>
          <div style="text-align:center;margin:28px 0">
            <a href="${resetUrl}" style="display:inline-block;padding:13px 22px;border-radius:10px;background:#67e8f9;color:#07101c;text-decoration:none;font-weight:700">Buat Password Baru</a>
          </div>
          <p style="line-height:1.7;color:#a8b3c7">Link ini berlaku selama 30 menit dan hanya dapat digunakan untuk mengganti password. Jika Anda tidak meminta reset password, abaikan email ini.</p>
        </div>
      </div>
    `,
  }
