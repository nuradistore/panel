  ...
  const handleVerifyEmail = async () => {
    ...
    try {
      if (email.trim().toLowerCase() !== transaction.email.toLowerCase()) {
        ...
        if (failCount >= 3) {
          setLocked(true)
          setError("Form dikunci karena 3x gagal. Coba lagi beberapa menit kemudian.")
          return
        }
        setError("Email tidak sesuai dengan transaksi ini.")
        return
      }

      const plan = plans.find((p) => p?.id === transaction.planId)
      if (!plan) return setError("Paket tidak ditemukan. Hubungi admin.")
      ...
      setPanelData({ username: transaction.username, password: panel.password, serverId: panel.serverId })
            ...
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-[28px] border border-cyan-300/10 bg-[#0A1020] shadow-[0_30px_90px_rgba(0,0,0,.35)]">
              ...
              <div className="p-5 sm:p-7">
                ...
                <div className="mt-6 grid gap-2 sm:grid-cols-2">
                  ...
                  <Info label="Email" value={maskEmail(transaction.email)} />
