...
export async function registerAction(formData: FormData) {
  ...
  if (password !== confirmPassword) redirect(`/register?error=${enc("Konfirmasi password tidak sesuai.")}`)
...
export async function loginAction(formData: FormData) {
  ...
  if (!result) redirect(`/login?error=${enc("Email/username atau password salah.")}`)
  ...
  redirect("/store?product=panel-bot")
}
