...
import { countAlightMotionStock, getAlightMotionProduct } from "@/lib/alight-motion"
...
export async function createPayment(
  ...
  try {
    ...
    const isAlightMotion = category === "alight-motion"

    const panelPlan = !isRedfinger && !isAlightMotion ? plans.find((p) => p?.id === planId) : undefined

    const redfingerProduct = isRedfinger
      ? await getRedfingerProduct(planId)
      : null

    const alightMotionProduct = isAlightMotion ? await getAlightMotionProduct(planId) : null

    if (!isRedfinger && !isAlightMotion && !panelPlan) {
    ...
    if (isAlightMotion && !alightMotionProduct) throw new Error("Produk AM Premium tidak ditemukan")
    if (isAlightMotion && await countAlightMotionStock(planId) <= 0) throw new Error("Stok AM Premium sedang habis")
    ...
    const price = isRedfinger ? redfingerProduct!.price : isAlightMotion ? alightMotionProduct!.price : panelPlan!.price

    const productName = isRedfinger ? redfingerProduct!.name : isAlightMotion ? alightMotionProduct!.name : panelPlan!.name
    ...
    bodyData.append(
      "name",
      isRedfinger ? "Pelanggan REDFINGER" : isAlightMotion ? "Pelanggan AM Premium" : cleanUsername,
    )
    ...
    bodyData.append(
      "phone",
      isRedfinger ? cleanPhone : "6280000000000",
    )
    ...
    const paymentData:
      ...
      productType: isRedfinger ? "redfinger" : isAlightMotion ? "alight-motion" : "panel",

      productName,

      duration: isRedfinger ? redfingerProduct!.duration : isAlightMotion ? alightMotionProduct!.duration : undefined,
