...
export async function createPayment(
  ...
  try {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(cleanEmail)) throw new Error("Email tidak valid")
    if (!SAKURU_API_ID || !SAKURU_API_KEY) throw new Error("Konfigurasi Sakurupiah belum lengkap")

    const isRedfinger = category === "redfinger"
    const panelPlan = !isRedfinger ? plans.find((p) => p?.id === planId) : undefined
    const redfingerProduct = isRedfinger ? await getRedfingerProduct(planId) : null

    if (!isRedfinger && !panelPlan) throw new Error("Plan tidak ditemukan")
    if (isRedfinger && !redfingerProduct) throw new Error("Produk REDFINGER tidak ditemukan")
    ...
    if (isRedfinger) {
      ...
      if (stock <= 0) throw new Error("Stok REDFINGER sedang habis")
    } else {
    ...
    const price = isRedfinger ? redfingerProduct!.price : panelPlan!.price
    const productName = isRedfinger ? redfingerProduct!.name : panelPlan!.name
    ...
    bodyData.append("name", isRedfinger ? "Pelanggan REDFINGER" : cleanUsername)
    bodyData.append("email", cleanEmail)
    bodyData.append("phone", isRedfinger ? cleanPhone : "6280000000000")
    ...
    bodyData.append("harga[]", price.toString())
    ...
    if (String(json.status) !== "200") throw new Error(json.message || "Gagal membuat invoice Sakurupiah")

    const pay = json.data?.[0]
    if (!pay?.trx_id || !pay?.qr || !pay?.expired) throw new Error("Data pembayaran Sakurupiah tidak lengkap")
    ...
    const paymentData: PaymentData = {
      ...
      planId,
      productType: isRedfinger ? "redfinger" : "panel",
      productName,
      duration: isRedfinger ? redfingerProduct!.duration : undefined,
      username: cleanUsername,
      phone: isRedfinger ? cleanPhone : undefined,
      email: cleanEmail,
      amount: price,
      fee: internalFee,
      total: nominal,
