...
export async function GET() {
  try {
    ...
    const [totalPurchases, codeSold] = await Promise.all([
    ...
    return NextResponse.json(
      { totalUsers, totalServers, totalPurchases, codeSold },
      { headers: { "Cache-Control": "no-store, no-cache, must-revalidate" } },
    )
  } catch (error) {
    console.error("Error fetching stats:", error)
    return NextResponse.json(
      { totalUsers: 0, totalServers: 0, totalPurchases: 0, codeSold: 0, error: "Failed to fetch stats" },
