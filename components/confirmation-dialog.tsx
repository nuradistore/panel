...
interface ConfirmationDialogProps {
  ...
  alightMotionProduct?: AlightMotionProductWithStock
}

export function ConfirmationDialog({ open, onOpenChange, planId, onConfirm, isLoading, category, redfingerProduct, alightMotionProduct }: ConfirmationDialogProps) {
  const isRedfinger = category === "redfinger"
  const isAlightMotion = category === "alight-motion"
  const plan = !isRedfinger && !isAlightMotion ? plans.find((p) => p?.id === planId) : undefined
  if (isRedfinger && !redfingerProduct) return null
  if (isAlightMotion && !alightMotionProduct) return null
  if (!isRedfinger && !isAlightMotion && !plan) return null

  const name = isRedfinger ? redfingerProduct!.name : isAlightMotion ? alightMotionProduct!.name : plan!.name
  const price = isRedfinger ? redfingerProduct!.price : isAlightMotion ? alightMotionProduct!.price : plan!.price
  const description = isRedfinger ? redfingerProduct!.description : isAlightMotion ? alightMotionProduct!.description : plan!.description
        ...
        <div className="p-5 sm:p-6">
          ...
          <div className="mt-5 rounded-[22px] border border-white/8 bg-[#070B15] p-4">
            ...
            {isRedfinger ? (
              ...
              <div className="mt-4 grid grid-cols-2 gap-2">
                <Mini icon={<Database className="h-3.5 w-3.5" />} label="Masa Aktif" value={alightMotionProduct!.duration} />
                <Mini icon={<Database className="h-3.5 w-3.5" />} label="Stok" value={alightMotionProduct!.stock > 0 ? `${alightMotionProduct!.stock} tersedia` : "STOK HABIS"} />
              </div>
