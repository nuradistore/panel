    ...
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3, delay: 0.1 }}>
      <Card className="overflow-hidden border-cyan-300/10 bg-[#0A1020]">
        ...
        <CardContent className="p-0">
          ...
          <div className="border-b border-white/8 p-5">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div className="flex items-start space-x-3">
                <div className="mt-1 rounded-full bg-[#070B15] p-2">{redfinger ? <Phone className="h-4 w-4 text-cyan-300" /> : <User className="h-4 w-4 text-cyan-300" />}</div>
                <div><p className="text-sm text-gray-400">{redfinger ? "Nomor WhatsApp" : "Nama Pengguna"}</p><p className="font-medium text-white">{redfinger ? phone || "-" : username}</p></div>
              </div>
          ...
          <div className="border-b border-white/8 p-5">
            <h4 className="mb-3 text-sm font-medium uppercase text-gray-400">{redfinger ? "Detail Produk" : "Detail Paket"}</h4>
