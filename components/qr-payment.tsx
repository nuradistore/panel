...
export function QrPayment({
  ...
  const getPlanName = () => {
    ...
    return plans.find((p) => p?.id === planId)?.name || (isRedfinger ? "REDFINGER" : "Unknown")
  }
  ...
  const applyResult = (result: any, showFeedback: boolean) => {
    ...
    if (result.status === "completed") {
      setStatus("completed")
      if (isRedfinger) {
        setRedfingerDetails(result.redfingerDetails || null)
        saveTransactionToHistory("completed", result.redfingerDetails)
      } else {
      ...
      if (showFeedback) {
        ...
        setModalMessage(isRedfinger ? "Redeem Code REDFINGER sudah siap dan dikirim ke email Anda." : "Panel Anda telah berhasil dibuat dan detail akun dikirim ke email Anda.")
        setShowModal(true)
      }
      if (!isRedfinger && result.showWhatsappPopup) setTimeout(() => setShowWhatsappPopup(true), 1000)
      ...
      if (showFeedback) {
        ...
        setModalMessage(isRedfinger ? "Pembayaran diterima. Redeem Code sedang diproses." : "Pembayaran diterima. Panel sedang dalam proses pembuatan.")
        setShowModal(true)
      }
  ...
  return (
    <div className="relative space-y-6">
      ...
      {!isRedfinger && <WhatsappGroupPopup isOpen={showWhatsappPopup} onClose={() => setShowWhatsappPopup(false)} />}
      ...
      {status === "completed" && (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.3 }}>
          <Card className="overflow-hidden border-cyan-300/10 bg-[#0A1020]">
            ...
            <CardContent className="p-6">
              <div className="mb-6 text-center"><p className="text-gray-300">{isRedfinger ? "Terima kasih atas pembelian Anda. Redeem Code REDFINGER telah diproses dan juga dikirim ke email Anda." : "Terima kasih atas pembelian Anda. Panel Pterodactyl telah berhasil dibuat dan detail akun telah dikirim ke email Anda."}</p></div>
