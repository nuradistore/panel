...
import { ArrowDownRight, Bot, Cloud, Crown, ShieldCheck, Zap } from "lucide-react"

const hero = {
  "panel-bot": {
    eyebrow: "Instant panel service",
    title: "Panel cepat.",
  ...
  redfinger: {
    eyebrow: "REDFINGER Cloud",
    title: "Redeem cepat.",
    accent: "Code langsung terkirim.",
    text: "Pilih REDFINGER VIP 7 Hari atau 30 Hari, isi nomor WhatsApp dan email aktif. Setelah pembayaran berhasil, redeem code tampil di invoice dan dikirim ke email.",
  },
} satisfies Record<StoreCategory, { eyebrow: string; title: string; accent: string; text: string }>

export function StorefrontHome() {
  const [activeCategory, setActiveCategory] = useState<StoreCategory>("panel-bot")
      ...
      <section className="relative overflow-hidden border-b border-white/5 pt-28 pb-16 md:pt-36 md:pb-24">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 px-4 md:px-6 lg:grid-cols-[0.8fr_1.2fr] lg:items-start">
          <div className="lg:sticky lg:top-28">
            ...
            <p className="mt-5 max-w-lg text-sm leading-7 text-slate-400 md:text-base">{copy.text}</p>

            <div className="mt-8 grid max-w-lg grid-cols-3 gap-3">
              <MiniStat icon={activeCategory === "redfinger" ? <Cloud className="h-4 w-4" /> : activeCategory === "admin-panel" ? <Crown className="h-4 w-4" /> : <Bot className="h-4 w-4" />} label={activeCategory === "redfinger" ? "Code Ready" : activeCategory === "admin-panel" ? "Admin Ready" : "Bot Ready"} />
            ...
            <a href="#order" className="mt-8 inline-flex items-center gap-2 text-sm font-semibold text-white hover:text-cyan-200">
              Pilih produk sekarang <ArrowDownRight className="h-4 w-4" />
            </a>
          </div>

          <div id="order" className="scroll-mt-28">
            <PanelForm activeCategory={activeCategory} onCategoryChange={setActiveCategory} />
          </div>
        </div>
      </section>
