...
export function StatsSection({ category }: { category: StoreCategory }) {
  const [stats, setStats] = useState<StatsData>({ totalUsers: 0, totalServers: 0, totalPurchases: 0, codeSold: 0, amSold: 0 })

  useEffect(() => {
    fetch("/api/stats", { cache: "no-store" }).then((r) => r.json()).then(setStats).catch(() => {})
  }, [category])

  const items = category === "alight-motion" ? [{ icon: ShoppingBag, label: "AM Premium Terjual", value: stats.amSold }] : category === "redfinger"
    ? [
  ...
  return (
    <section className="border-b border-white/5 bg-[#070A10] py-16">
      <div className="mx-auto max-w-7xl px-4 md:px-6">
        <div className={`grid gap-3 ${category === "alight-motion" ? "grid-cols-1" : category === "redfinger" ? "md:grid-cols-2" : "md:grid-cols-3"}`}>
