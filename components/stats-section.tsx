...
interface StatsData { totalUsers: number; totalServers: number; totalPurchases: number }

export function StatsSection() {
  const [stats, setStats] = useState<StatsData>({ totalUsers: 0, totalServers: 0, totalPurchases: 0 })
            ...
            return (
              <motion.div key={item.label} initial={{ opacity: 0, scale: .98 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: index * .08 }} className="rounded-3xl border border-white/7 bg-gradient-to-br from-white/[0.035] to-transparent p-6">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-cyan-300/15 bg-cyan-300/5 text-cyan-200">
