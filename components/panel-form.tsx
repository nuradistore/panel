...
const categoryMeta: Record<StoreCategory, { label: string; description: string; icon: React.ReactNode }> = {
  "panel-bot": {
    label: "Panel Bot",
  ...
  "admin-panel": {
    label: "Admin Panel",
    description: "Paket khusus akses dan pengelolaan administrator Pterodactyl.",
    icon: <Crown className="h-4 w-4" />,
  },
  redfinger: {
    label: "Code REDFINGER",
    description: "Redeem Code REDFINGER Cloud VIP dengan stok real-time dan pengiriman otomatis.",
...
export default function PanelForm({ activeCategory, onCategoryChange }: PanelFormProps) {
  const [username, setUsername] = useState("")
  const [phone, setPhone] = useState("")
  const [email, setEmail] = useState("")
  const [selectedPlan, setSelectedPlan] = useState("")
  const [redfingerProducts, setRedfingerProducts] = useState<RedfingerProductWithStock[]>([])
  ...
  const visiblePlans = useMemo(
    () => (activeCategory === "redfinger" ? [] : plans.filter((plan) => plan?.category === activeCategory)),
    [activeCategory],
  )

  const selectedPanelPlan = plans.find((plan) => plan?.id === selectedPlan)
  const selectedRedfinger = redfingerProducts.find((product) => product.productId === selectedPlan)
  const selectedName = selectedRedfinger?.name || selectedPanelPlan?.name || ""
  const selectedPrice = selectedRedfinger?.price ?? selectedPanelPlan?.price ?? 0

  const loadRedfinger = async () => {
    setLoadingRedfinger(true)
    try {
      const response = await fetch("/api/redfinger/products", { cache: "no-store" })
      const data = await response.json()
      setRedfingerProducts(Array.isArray(data.products) ? data.products : [])
    } catch {
  ...
  useEffect(() => {
    if (activeCategory === "redfinger") loadRedfinger()
  }, [activeCategory])
  ...
  const validateAndContinue = async () => {
    if (!selectedPlan) {
      toast({ title: "Produk belum dipilih", description: "Pilih produk terlebih dahulu.", variant: "destructive" })
      return
    }
    ...
    if (activeCategory === "redfinger") {
      ...
      if (!/^(?:08|628)\d{8,13}$/.test(cleanPhone)) {
        toast({ title: "Nomor WhatsApp tidak valid", description: "Gunakan nomor aktif, contoh 081234567890.", variant: "destructive" })
        return
      }
      if (!selectedRedfinger || selectedRedfinger.stock <= 0) {
        toast({ title: "Stok habis", description: "Pilih paket REDFINGER yang masih tersedia.", variant: "destructive" })
        return
      }
    ...
    if (username.length < 3 || /[^a-zA-Z0-9]/.test(username)) {
      toast({ title: "Username belum valid", description: "Minimal 3 karakter dan hanya boleh huruf serta angka.", variant: "destructive" })
      return
    }
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const customerValue = activeCategory === "redfinger" ? phone : username
      const result = await Promise.race([createPayment(selectedPlan, customerValue, email, activeCategory), timeout])

      if (!result.success || !result.transactionId) throw new Error(result.error || "Pembayaran belum dapat dibuat.")
      setShowConfirmation(false)
  ...
  return (
    <>
        ...
        <div className="border-b border-white/8 bg-[radial-gradient(circle_at_top_left,rgba(34,211,238,.12),transparent_45%),radial-gradient(circle_at_top_right,rgba(139,92,246,.10),transparent_42%)] p-5 md:p-7">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <h2 className="mt-1 text-2xl font-black tracking-tight text-white">Pilihan jenis produk</h2>
            </div>
        ...
        <div className="space-y-6 p-4 md:p-7">
          <div className="grid gap-3 md:grid-cols-2">
            {activeCategory === "redfinger" ? (
              <Field icon={<Phone className="h-4 w-4" />} label="Nomor WhatsApp">
                <Input
                  value={phone}
          ...
          <div>
            <div className="grid grid-cols-3 gap-1.5 rounded-2xl border border-white/8 bg-[#070B15] p-1.5">
              {(Object.keys(categoryMeta) as StoreCategory[]).map((category) => {
          ...
          {activeCategory === "redfinger" ? (
            loadingRedfinger ? (
            ...
            ) : redfingerProducts.length > 0 ? (
              <motion.div key="redfinger" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-2 gap-2.5 lg:gap-3">
                {redfingerProducts.map((product) => {
                  const active = selectedPlan === product.productId
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      disabled={soldOut}
                      onClick={() => setSelectedPlan(product.productId)}
                      ...
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <div className="flex items-center gap-1.5">
                            ...
                            <h3 className="line-clamp-2 text-[12px] font-black leading-4 text-white sm:text-sm">{product.name}</h3>
                          </div>
                          <p className="mt-2 text-xl font-black text-cyan-200 sm:text-2xl">{formatRupiah(product.price)}</p>
                        </div>
                        {active && <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-cyan-300 text-[#041017]"><Check className="h-4 w-4" /></div>}
                      </div>

                      <div className="mt-4 grid grid-cols-2 gap-2 text-[10px] sm:text-[11px]">
                        <Mini label="Masa Aktif" value={product.duration} />
                        <Mini label="Stok" value={soldOut ? "HABIS" : `${product.stock} tersedia`} />
                      </div>
                      <p className="mt-3 line-clamp-2 text-[10px] leading-4 text-slate-500 sm:text-[11px]">{product.description}</p>
                      {soldOut && <span className="absolute bottom-3 right-3 rounded-full border border-rose-300/15 bg-rose-300/[0.06] px-2 py-1 text-[9px] font-black text-rose-300">STOK HABIS</span>}
                    </button>
                  )
                })}
              </motion.div>
              ...
              <div className="rounded-[22px] border border-dashed border-cyan-300/15 bg-cyan-300/[0.025] px-5 py-10 text-center">
                ...
                <h3 className="mt-3 text-sm font-bold text-white">Produk REDFINGER belum tersedia</h3>
                <p className="mt-1 text-xs text-slate-500">Pastikan koneksi MongoDB aktif lalu muat ulang halaman.</p>
              </div>
            )
          ) : visiblePlans.length > 0 ? (
            <motion.div key={activeCategory} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.18 }} className="grid grid-cols-2 gap-2.5 lg:gap-3">
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <p className="mt-2 text-xl font-black text-cyan-200 sm:text-2xl">{formatRupiah(plan.price)}</p>
                      </div>
                      {active && <div className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-cyan-300 text-[#041017]"><Check className="h-4 w-4" /></div>}
                    </div>

                    {plan.category === "admin-panel" ? (
                      <div className="mt-4 rounded-xl border border-violet-300/10 bg-violet-300/[0.035] p-3 text-[10px] text-slate-400 sm:text-[11px]">Akses Administrator Pterodactyl</div>
                    ) : (
                      <div className="mt-4 grid grid-cols-3 gap-1.5">
                        <MiniIcon icon={<Database className="h-3.5 w-3.5" />} label="RAM" value={plan.memory === 0 ? "∞" : `${plan.memory} MB`} />
                        <MiniIcon icon={<HardDrive className="h-3.5 w-3.5" />} label="Disk" value={plan.disk === 0 ? "∞" : `${plan.disk} MB`} />
                        <MiniIcon icon={<Cpu className="h-3.5 w-3.5" />} label="CPU" value={plan.cpu === 0 ? "∞" : `${plan.cpu}%`} />
                      </div>
                    )}
          ...
          <Button
            type="submit"
            disabled={isValidating || !selectedPlan || (activeCategory === "redfinger" && !!selectedRedfinger && selectedRedfinger.stock <= 0)}
        ...
        <div className="fixed inset-x-0 bottom-0 z-40 border-t border-cyan-300/15 bg-[#070B15]/95 p-3 shadow-[0_-18px_60px_rgba(0,0,0,.45)] backdrop-blur-xl sm:hidden">
          <div className="mx-auto flex max-w-xl items-center gap-3">
            <div className="min-w-0 flex-1">
              <p className="truncate text-[11px] font-bold text-white">{selectedName}</p>
              <p className="text-base font-black text-cyan-200">{formatRupiah(selectedPrice)}</p>
            </div>
            <Button type="button" onClick={validateAndContinue} disabled={isValidating} className="h-11 rounded-xl bg-gradient-to-r from-cyan-300 to-sky-400 px-4 font-black text-[#041017]">
      ...
      <StatusModal isOpen={showModal} onClose={() => !isLoading && setShowModal(false)} type={modalType} title={modalTitle} message={modalMessage} />
      ...
      <ConfirmationDialog
        ...
        redfingerProduct={selectedRedfinger}
      />
    </>
  )
}
...
function Mini({ label, value }: { label: string; value: string }) {
  return <div className="rounded-xl border border-white/6 bg-black/20 p-2"><div className="text-[9px] text-slate-600">{label}</div><div className="mt-1 truncate text-[10px] font-bold text-slate-300 sm:text-[11px]">{value}</div></div>
}

function MiniIcon({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return <div className="rounded-xl border border-white/6 bg-black/20 p-2"><div className="text-cyan-300">{icon}</div><div className="mt-1 text-[9px] text-slate-600">{label}</div><div className="truncate text-[9px] font-bold text-slate-300 sm:text-[10px]">{value}</div></div>
}
