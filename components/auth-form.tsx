      ...
      <div className="mx-auto max-w-md">
        ...
        <div className="rounded-[2rem] border border-white/10 bg-white/[.025] p-6 shadow-2xl md:p-8">
          ...
          <form action={formAction} className="space-y-4">
            ...
            {mode === "login" ? (
              <Field name="identifier" label="Email / Username" type="text" icon={<Mail className="h-4 w-4" />} />
            ) : (mode === "register" || mode === "forgot") ? (
              <Field name="email" label="Email Aktif" type="email" icon={<Mail className="h-4 w-4" />} />
            ) : null}

            {(mode === "login" || mode === "register" || mode === "reset") && <PasswordField name="password" label={mode === "reset" ? "Password Baru" : "Password"} autoComplete={mode === "register" || mode === "reset" ? "new-password" : "current-password"} />}

            {mode === "register" && <PasswordField name="confirmPassword" label="Konfirmasi Password" autoComplete="new-password" />}
          ...
          {(mode === "login" || mode === "register") && (
            <Link href="/store?product=panel-bot" className="click-motion mt-4 block text-center text-xs text-slate-500 hover:text-white">
