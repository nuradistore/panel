    ...
    <section id="faq" className="bg-[#090D14] py-20">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 md:px-6 lg:grid-cols-[0.7fr_1.3fr]">
        <div>
          ...
          <p className="mt-4 max-w-sm text-sm leading-7 text-slate-500">Jawaban cepat untuk hal yang paling sering ditanyakan sebelum membeli panel.</p>
        </div>
                ...
                <AnimatePresence>
                  {active && (
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden">
                      <p className="px-5 pb-5 text-xs leading-6 text-slate-500">{faq.answer}</p>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
